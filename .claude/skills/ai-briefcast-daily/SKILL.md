...
- **★ MG 动画版走 ai-briefcast-mg 技能（2026-07-22 起）**：用户说"MG 版/动画版/ChatCut 版"时，选题写稿仍按本技能，出片换 ChatCut 手写 MG（TTS 音色/读法坑、whisper 卡点、字幕 MG、封面仍用本技能 make_cover 单出，全在该技能里）。
...
- **可跨日期混合**：常见需求是"某天的新闻 + 另一天的论文"，且按 **N 闻 + M 论文** 选（**2026-07-22 起默认 3 新闻 + 1 论文**，用户点名定的）。
...
- **★ 文风铁律（2026-06-23 用户多轮打磨定稿，以后照这个写，别再返工）。范本＝`output/2026-06-23/scripts/broadcast-0623-B-changed.md`**：
  ...
  11. 默认 **3 新闻 + 1 论文**（2026-07-22 用户定，此前是 3+2），用户没说就别偷偷加量；选题**逐条核发布日期**（聚合站爱把旧闻当"持续报道"混进当日榜，旧的一律踢）。
...
```
读 source，确认 **N 闻 + M 论文**（默认 3 头条 + 1 篇论文，2026-07-22 起）。论文页常比资讯晚 1 天才更新。
...
当用户只说"正文 tag"时，直接输出：
- `正文`：无编号词"第一条/第二条"，用短句逐条覆盖当天条目（默认 3 新闻 + 1 论文），结尾带 `@柿子树下的猫wanjeans`。
