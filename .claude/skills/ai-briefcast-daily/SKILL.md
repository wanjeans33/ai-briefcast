...
- **语速可调**：`doubao_tts_ws` 读 `VOLC_SPEECH_RATE`（正数更快）。实测 rate 30≈1.19×、60≈1.51×、
  上限约 1.85×。想要更短的视频先调语速；但**只靠语速到不了大幅缩短**（如 ~150s 稿拉满也才 ~80s），
  真要 1 分钟还得删稿。
...
- **语速**：合成前设 `os.environ["VOLC_SPEECH_RATE"]="60"`（≈1.5×，自然偏快）等；run_tts 会把它传进豆包。
...
`BIG` 的数字＝内容卡数）。渲染后 **`git checkout scripts/make_cover.py` 还原模板**（避免把某天内容留在模板里）。
输出 3:4 = 1080×1440（2x → 2160×2880）。封面可单独上传为视频自定义封面。5 行也放得下。
