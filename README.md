...
python scripts/run_daily.py --video                # 额外产出一支小红书竖屏视频（9:16）
```

**小红书视频**：`--video` 会在出音频后，用 LLM 把当天素材改成卡片文案（`make_cards`），按 open-design `card-xiaohongshu` 的视觉渲成 9:16 卡片（Playwright 无头 Chromium），再用 ffmpeg 交叉淡化拼接 + 混入 concise 音频 → `audio_output/xhs-<日期>-concise.mp4`。需先装好 Playwright Chromium 与 ffmpeg。每日定时任务默认已带 `--video`。
...
python scripts/run_daily.py --video                # also produce a 9:16 Xiaohongshu video
```

**Xiaohongshu video**: `--video` turns the day's material into card copy via the LLM (`make_cards`), renders 9:16 cards in the open-design `card-xiaohongshu` style (headless Chromium), and composites them with ffmpeg crossfades + the concise audio into `audio_output/xhs-<date>-concise.mp4`. Requires Playwright Chromium + ffmpeg. The daily scheduled task includes `--video` by default.
